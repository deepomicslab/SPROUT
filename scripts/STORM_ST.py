...
weight = pd.read_csv(args.w_file,sep='\t',header=0 )
st_exp = pd.read_csv(args.st_file,sep='\t',header=0,index_col = 0 )
meta_df = pd.read_csv(args.meta_file,sep='\t',header=0,index_col = 0 )
...
def embedding(sparse_A, path, left_range = 0, right_range = 30, steps = 30, dim = 2):
...
ans, sc_spot_neighbor = optimizers.dig_hole_spatial_info(st_exp, picked_index_df, r = 2)
...
coord = embedding(sparse_A, path, left_range, right_range, steps, dim)
print('Finished!')
